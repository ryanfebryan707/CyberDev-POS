...
function AdminView({ onLogout }: { onLogout: () => void }) {
  ...
  const deleteClient=async()=>{
    ...
    setBusy(`delete-${target.id}`);setAdminMessage("");
    try{
      const response=await fetch("/api/admin/clients",{method:"DELETE",headers:{"content-type":"application/json"},body:JSON.stringify({tenantId:target.id,confirmation:deleteConfirmation})});
      const result=await response.json().catch(()=>({error:"Respons server tidak valid."})) as {error?:string};
      if(!response.ok){setAdminMessage(result.error||"Client gagal dihapus.");return;}
      setDeleteTarget(null);setDeleteConfirmation("");
      setAdminMessage(`Client ${target.name} beserta seluruh datanya berhasil dihapus permanen.`);
      if(data.clients.length===1&&page>1)setPage(value=>Math.max(1,value-1));else await load();
    }catch{
      setAdminMessage("Client gagal dihapus karena koneksi terputus. Silakan coba kembali.");
    }finally{
      setBusy("");
    }
  };
  ...
  return <div className="view-stack">
    ...
    <AlertDialog open={!!deleteTarget} onOpenChange={open=>{if(!open){setDeleteTarget(null);setDeleteConfirmation("");}}}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Hapus permanen client {deleteTarget?.name}?</AlertDialogTitle><AlertDialogDescription>Seluruh akun owner/karyawan, tautan Google, sesi login, produk, pelanggan, transaksi, pembayaran, paket, dan data toko akan dihapus. Tindakan ini tidak dapat dibatalkan.</AlertDialogDescription></AlertDialogHeader><label className="reset-confirm-label">Ketik nama toko <strong>{deleteTarget?.name}</strong><input value={deleteConfirmation} onChange={event=>setDeleteConfirmation(event.target.value)} placeholder={deleteTarget?.name||"Nama toko"}/></label><AlertDialogFooter><AlertDialogCancel>Batal</AlertDialogCancel><AlertDialogAction className="danger-action" disabled={!deleteTarget||deleteConfirmation!==deleteTarget.name||busy.startsWith("delete-")} onClick={deleteClient}>{busy.startsWith("delete-")?"Menghapus...":"Hapus client permanen"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
...
function LoginView({ onAuthenticated }: { onAuthenticated: (user: AppUser) => void }) {
  ...
  const normalizedClientEmail = identifier.trim().toLowerCase();
  const googleLoginHint = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedClientEmail) ? normalizedClientEmail : "";
  return <main className="login-shell">
    <section className="login-panel">
      ...
      <div className="login-form-wrap">
        ...
        <p>{mode === "register" ? "Isi data secara manual terlebih dahulu. Toko, akun owner, dan Demo 14 hari langsung dibuat; sesudah itu email yang sama dapat memakai Login Google." : mode==="admin" ? "Portal terpisah untuk pemilik CyberDev POS memantau client, pembayaran, lokasi, IP, dan masa langganan." : "Masuk ke toko yang sudah didaftarkan sendiri atau ditambahkan Admin. Login Google hanya menerima email Client yang telah terdaftar."}</p>
        ...
        {mode === "client"&&<>{googleLoginHint?<GoogleIdentityButton disabled={loading} loginHint={googleLoginHint} onSuccess={finishGoogleLogin}/>:<p className="google-registration-note">Isi alamat email Client yang terdaftar pada kolom di atas untuk menampilkan Login Google.</p>}<p className="google-registration-note">Google hanya dapat masuk dengan alamat email yang sama persis seperti pendaftaran Demo manual atau data yang dibuat oleh Admin.</p></>}
